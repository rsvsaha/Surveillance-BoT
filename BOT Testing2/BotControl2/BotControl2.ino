...
#define MOTOSPEED  3000 //Represents the delay
...
DDRC=0x00111100b; //A2-A5 as OUTPUT A5-Green A4-Purple A3-Grey A2-Yellow
...
void Forward()
{
 ...
 if(!dangerFront)
  {
    buf=0x11000011b&PORTC; //MASK THE FIRST AND LAST TWO BITS AND CLEARING THE MIDDLE 4 BITS FOR NEW OUTPUT
    buf=buf|0x00010100b; //SETTING THE MIDDLE FOUR BITS AS PER MOVEMENT REQUIREMENT
...
void Backward()
{
 ...
 if(!dangerBack)
  {
    buf=0x11000011b&PORTC; //MASK THE FIRST AND LAST TWO BITS AND CLEARING THE MIDDLE 4 BITS FOR NEW OUTPUT
    buf=buf|0x00101000b; //SETTING THE MIDDLE FOUR BITS AS PER MOVEMENT REQUIREMENT
    PORTC=buf; //LOADING THE DATA TO PORT
    delay(MOTOSPEED);//WAIT FOR THE MOTOR TO ROTATE
    buf=0x11000011b&PORTC; //MASK THE FIRST AND LAST TWO BITS AND CLEARING THE MIDDLE 4 BITS FOR NEW OUTPUT
    PORTC=buf;
...
void Left() //MAKES IT MOVE LEFT CIRCLE
{
  Serial.println("LEFT");
    ...
    buf=0x11000011b&PORTC; //MASK THE FIRST AND LAST TWO BITS AND CLEARING THE MIDDLE 4 BITS FOR NEW OUTPUT
    buf=buf|0x00100100b; //SETTING THE MIDDLE FOUR BITS AS PER MOVEMENT REQUIREMENT
    PORTC=buf; //LOADING THE DATA TO PORT
    delay(MOTOSPEED);//WAIT FOR THE MOTOR TO ROTATE
    buf=0x11000011b&PORTC; //MASK THE FIRST AND LAST TWO BITS AND CLEARING THE MIDDLE 4 BITS FOR NEW OUTPUT
    PORTC=buf;
...
void Right() //MAKES IT MOVE RIGHT CIRCLE
{
  ...
  Serial.println("RIGHT");
    ...
    buf=0x11000011b&PORTC; //MASK THE FIRST AND LAST TWO BITS AND CLEARING THE MIDDLE 4 BITS FOR NEW OUTPUT
    buf=buf|0x00011000b; //SETTING THE MIDDLE FOUR BITS AS PER MOVEMENT REQUIREMENT
    PORTC=buf; //LOADING THE DATA TO PORT
    delay(MOTOSPEED);//WAIT FOR THE MOTOR TO ROTATE
    buf=0x11000011b&PORTC; //MASK THE FIRST AND LAST TWO BITS AND CLEARING THE MIDDLE 4 BITS FOR NEW OUTPUT
    PORTC=buf;
